...
public class MSTInDirectedGraph {
	...
	public static void main(String[] args) {
		...
		new MSTInDirectedGraph().directedMSTEdmondImpl("input.txt");
		}
	...
	public void FindAndAddCycles()
	{
		...
		{
			ArrayList<Edge> minOutEdges=minInEdgeGraph.getOutEdges(i);
			if(minOutEdges!=null)
			{
			...
			if(!visited[i])
			{
			cycleExistCheck(minInEdgeGraph, e.u,e.u);
			cycleList.add(cycle);
			cycle=new ArrayList<Integer>();
			}
			}
	...
	public void gatherEdgesEnterAndPresentInCycle()
	{
		...
		for(int n:cycle)
		{
			...
			{
				//if()
				Edge o=minInEdgeGraph.getInEdges(n, noOfVertices).get(0);
	...
	public void modifyPseudoEdgeInCycle()
	{
			...
			if(minAdjustedEnteringEdge!=null)
